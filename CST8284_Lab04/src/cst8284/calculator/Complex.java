...
public class Complex {
	...
	public boolean equals(int real, int	imag) {
		return (double)real == this.real && (double)imag == this.imag;
		
	}
	public boolean equals(double real, double imag, double delta) {
	// return Math.abs(c1.real, c2.real, delta) && Math.abs(c1.imag, c2.imag, delta);
	return Math.abs(this.real - getReal()) < delta && Math.abs(this.imag - getImag()) < delta;
	// return (delta > Math.abs(real-imag));
	}
	public boolean equals(String c) {
	String cSplit[] = c.split("(?=\\+)|(?=\\-)");
	return ((this.getReal() == Integer.parseInt(cSplit[0])) && (this.getImag() == Integer.parseInt(cSplit[1].replace("i", ""))));
	}
